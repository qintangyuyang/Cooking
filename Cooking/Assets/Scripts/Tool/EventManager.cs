...
public static class EventManager
{
    ...
    public static void TriggerEvent(EventType eventType)
    {
        if (eventTable.TryGetValue(eventType, out Delegate eventRegistrationToken))
        {
            ...
            {
                Debug.LogError($"TriggerEvent失败，事件类型{eventType},委托签名不匹配，期望委托签名：Action，实际注册委托：{eventRegistrationToken}");
            }
        }
    }
    ...
    public static void TriggerEvent<T>(EventType eventType, T arg)
    {
        if (eventTable.TryGetValue(eventType, out Delegate eventRegistrationToken))
        {
            ...
            {
                Debug.LogError($"TriggerEvent失败，事件类型{eventType},委托签名不匹配，期望委托签名：Action<{typeof(T).Name}>，实际注册委托：{eventRegistrationToken}");
            }
        }
    }
    ...
    public static void TriggerEvent<T1, T2>(EventType eventType, T1 arg1, T2 arg2)
    {
        if (eventTable.TryGetValue(eventType, out Delegate eventRegistrationToken))
        {
            ...
            {
                Debug.LogError($"TriggerEvent失败，事件类型{eventType},委托签名不匹配，期望委托签名：Action<{typeof(T1).Name}, {typeof(T2).Name}>，实际注册委托：{eventRegistrationToken}");
            }
        }
    }
}
